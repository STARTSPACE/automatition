...
sudo aws ec2 describe-instances --filters Name=tag:bash-stoptime,Values=05-00 Name=tag:bash-profile,Values=ad --query Reservations[*].Instances[*].[InstanceId] --output text > ~/tmp/stop_ad_instance_info.txt
