...
sudo aws ec2 describe-instances --filters Name=tag:bash-stoptime,Values=12-00 Name=tag:bash-profile,Values=wd --query Reservations[*].Instances[*].[InstanceId] --output text > ~/tmp/stop_wd_instance_info.txt
