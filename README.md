...
На следующей странице в полученном списке прав выберите те права, которые создали на предыдущем шаге (для ускорения поиска можно возпользоваться полем поиска, например используя "ec2-actions-all-instances").
...
Да, без ограничений. Мы их создавали как пример автоматизации использования инфраструктуры Amazon Web Services. Если Вы заинтересованы в более глубокой интеграции с Вашей инфосистемой, то связывайтесь с нами по электронной почте [```contact@startspace.ru```] (<a href="mailto:someone@example.com?Subject=Hello%20again" target="_top">Send Mail</a>), постараемся помочь.
