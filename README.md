...
```
$ find ~/aws/automation -type f -exec chmod +x {} \;
```
...
Это рекомендуемое действие, оно позволит Вам получать сообщения об ошибках на имя пользователя от которого запускаются скрипты на CRON. В данном случае установка предполагает только локальную доставку сообщений, но настройками может быть определен и другой путь доставки, например на Ваш адрес е-мейл или СМС.
- Установливаем службу ```sudo apt-get install postfix```
- Во время установки примите ```Local only``` и примите другие настройки
- Сообщения можно просматривать командой ```sudo nano /var/mail/ubuntu``` (Замените ```ubuntu``` на имя того пользователя, чью почту Вы хотите посмотреть.
...
- Сбрасываем изменения (иначе может возникнуть ошибка) ```$ git stash```
- Устанавливаем обновления ```$ git pull```
- Обновляем владельца обновленных файлов ```sudo find ~/aws/automation -not -path '*/\.*' -exec chown -R ubuntu:ubuntu {} \;``` (**ВНИМАНИЕ!** Значение ```ubuntu:ubuntu``` примнимо только для пользователя по умолчанию. Если скрипты настроены от другого пользователя, то замените значения на необходимые Вам.)
- Обновляем права на установленные обновления ```$ find ~/aws/automation -type f -exec chmod +x {} \;```
...
**Внимание! Обновление владельца файлов и прав на них не обязательное, но крайне рекомендуемое действие. Вреда не наносит, но позволит избежать проблем выполнения скриптов**
