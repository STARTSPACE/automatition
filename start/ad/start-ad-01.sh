...
sudo aws ec2 describe-instances --filters Name=tag:bash-starttime,Values=01-00 Name=tag:bash-profile,Values=ad --query Reservations[*].Instances[*].[InstanceId] --output text > ~/tmp/start_ad_instance_info.txt
