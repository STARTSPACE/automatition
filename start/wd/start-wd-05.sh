...
sudo aws ec2 describe-instances --filters Name=tag:bash-starttime,Values=05-00 Name=tag:bash-profile,Values=wd --query Reservations[*].Instances[*].[InstanceId] --output text > ~/tmp/start_wd_instance_info.txt
