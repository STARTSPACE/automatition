...
sudo aws ec2 describe-instances --filters Name=tag:start-time,Values=00-00 Name=tag:bash-profile,Values=wd --query Reservations[*].Instances[*].[InstanceId] --output text > ~/tmp/start_wd_instance_info.txt 2>&1
