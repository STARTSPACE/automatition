...
sudo aws ec2 describe-instances --filters Name=tag:bash-reboottime,Values=02-00 Name=tag:bash-profile,Values=ad --query Reservations[*].Instances[*].[InstanceId] --output text > ~/tmp/reboot_ad_instance_info.txt 2>&1
