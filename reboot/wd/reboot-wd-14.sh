...
sudo aws ec2 describe-instances --filters Name=tag:reboot-time,Values=14-00 Name=tag:bash-profile,Values=wd --query Reservations[*].Instances[*].[InstanceId] --output text > ~/tmp/reboot_wd_instance_info.txt 2>&1
